...
public class PresentableMessageMapperTests {
    ...
    RuleConsequence createRuleConsequence() {
        ...
        data.put(
                MessagingConstants.EventDataKeys.RulesEngine.MESSAGE_CONSEQUENCE_DETAIL_KEY_CONTENT,
                html);
        data.put(MessagingConstants.EventDataKeys.RulesEngine.MESSAGE_CONSEQUENCE_ID, "123456789");
        details.put(
                MessagingConstants.EventDataKeys.RulesEngine.MESSAGE_CONSEQUENCE_DETAIL_KEY_DATA,
                data);
        details.put(
                MessagingConstants.EventDataKeys.RulesEngine.MESSAGE_CONSEQUENCE_DETAIL_KEY_SCHEMA,
                MessagingConstants.SchemaValues.SCHEMA_IAM);
        return new RuleConsequence(
                "123456789",
                MessagingTestConstants.EventDataKeys.RulesEngine.MESSAGE_CONSEQUENCE_CJM_VALUE,
                details);
    }
    ...
    //        runUsingMockedServiceProvider(() -> {
    //            try {
    //                internalMessage = (PresentableMessageMapper.InternalMessage)
    // PresentableMessageMapper.getInstance().createMessage(mockMessagingExtension,
    // createRuleConsequence(), new HashMap<>(), new HashMap<>(), null);
    //            } catch (Exception exception) {
    //                fail(exception.getMessage());
    //            }
    //            // test
    ...
    //            verify(mockMessagingExtension,
    // times(1)).sendPropositionInteraction(interactionArgumentCaptor.capture(),
    // messagingEdgeEventTypeArgumentCaptor.capture(),
    // any(PresentableMessageMapper.InternalMessage.class));
    ...
    //            verify(mockMessagingExtension,
    // times(0)).sendPropositionInteraction(any(String.class), any(MessagingEdgeEventType.class),
    // any(PresentableMessageMapper.InternalMessage.class));
    //        });
    //    }
    //
    ...
    // messagingEdgeEventTypeArgumentCaptor.capture(),
    // any(PresentableMessageMapper.InternalMessage.class));
    //            MessagingEdgeEventType displayTrackingEvent =
    // messagingEdgeEventTypeArgumentCaptor.getValue();
    //            String interaction = interactionArgumentCaptor.getValue();
