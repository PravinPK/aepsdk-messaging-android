...
public class MessagingFullscreenEventListenerTests {
    ...
    public void test_onUrlLoading_nullUrlString() {
        runWithMockedServiceProvider(
                () -> {
                    // setup
                    when(((InAppMessage) mockPresentation).getSettings())
                            .thenReturn(mockMessageSettings);
                    ...
                    verify(mockMessage, times(0))
                            .track(anyString(), any(MessagingEdgeEventType.class));
                });
    }
    ...
    //            // setup
    //            when(((InAppMessage)
    // mockPresentation).getSettings()).thenReturn(mockMessageSettings);
    //            when(((InAppMessage)
    // mockPresentation).getEventHandler()).thenReturn(mockEventHandler);
    //
    //            // test
    //            eventListener.onUrlLoading(mockInAppPresentable,
    // "adbinapp://dismiss?interaction=javascript&link=js%3D%28function%28%29+%7B+return+%27javascript+value%27%3B+%7D%29%28%29%3B");
    //
    //            // verify no message tracking call and message settings weren't created
    //            verify(mockMessage, times(1)).track(eq("javascript"),
    // eq(MessagingEdgeEventType.IN_APP_INTERACT));
    //            ArgumentCaptor<String> stringArgumentCaptor =
    // ArgumentCaptor.forClass(String.class);
    //            verify(mockEventHandler,
    // times(1)).evaluateJavascript(stringArgumentCaptor.capture(), any());
    //            assertEquals("js=(function() { return 'javascript value'; })();",
    ...
    //
    //            // verify no message tracking call and message settings weren't created
    //            verify(mockMessage, times(0)).track(eq("deeplink"),
    // eq(MessagingEdgeEventType.IN_APP_DISMISS));
    //            ArgumentCaptor<String> stringArgumentCaptor =
    // ArgumentCaptor.forClass(String.class);
    //            verify(mockUriOpening, times(1)).openUri(stringArgumentCaptor.capture());
    //            assertEquals("scheme://parameters?param1=value1&param2=value2",
    // stringArgumentCaptor.getValue());
    //        });
    //    }
    //
    ...
    //
    //            // verify
    //            verify(mockMessage, times(1)).track("backPress",
    ...
    //
    //            // verify
    //            verify(mockMessage, times(0)).track("backPress",
