...
class PresentableMessageMapper {
     ...
     *
     * <p>Every {@code InternalMessage} requires a {@link RuleConsequence#getId()}, and must be of
     * type "cjmiam".
     *
     * <p>The consequence {@code Map} for a {@code InternalMessage} is required to have valid values
     * for the following fields:
     *
     * <ul>
     *   <li>{@value MessagingConstants.EventDataKeys.RulesEngine#MESSAGE_CONSEQUENCE_ID} - {@code
     *       String} containing the message ID
     *   <li>{@value MessagingConstants.EventDataKeys.RulesEngine#MESSAGE_CONSEQUENCE_TYPE} - {@code
     *       String} containing the consequence type
     *   <li>{@value MessagingConstants.EventDataKeys.RulesEngine#MESSAGE_CONSEQUENCE_DETAIL} -
     *       {@code Map<String, Object>} containing details of the Message to be displayed
     * </ul>
     *
     * @param messagingExtension {@link MessagingExtension} instance that created this Message
     * @param consequence {@link com.adobe.marketing.mobile.launch.rulesengine.RuleConsequence}
     *     containing a {@code InternalMessage} defining payload
     * @param rawInAppMessageSettings {@code Map<String, Object>} containing the raw message
     *     settings found in the "mobileParameters" present in the rule consequence
     * @param assetMap {@code Map<String, Object>} containing a mapping of a remote image asset URL
     *     and it's cached location
    ...
    Message createMessage(
            final MessagingExtension messagingExtension,
            ...
            throws MessageRequiredFieldMissingException, IllegalStateException {
        ...
        final InternalMessage internalMessage =
                new InternalMessage(
                        messagingExtension,
                        consequence,
                        rawInAppMessageSettings,
                        assetMap,
                        propositionInfo);
        presentableMessageMap.put(
                internalMessage.aepMessage.getPresentation().getId(), internalMessage);
    ...
    /**
     * Retrieves a {@code Message} object associated with the given presentableId from the
     * internally maintained map.
     *
     * @param presentableId The ID of the {@link Presentable} message to retrieve.
     * @return The {@link Message} object associated with the given presentableId, or null if no
     *     such message exists or the presentableId is null or empty.
     */
    @Nullable Message getMessageFromPresentableId(final String presentableId) {
    ...
    static class InternalMessage implements Message {
        ...
        // package private
        PropositionInfo
                propositionInfo; // contains XDM data necessary for tracking in-app interactions
        // with Adobe Journey Optimizer

        /**
         * Constructor.
         *
         * <p>Every {@link InternalMessage} requires a {@link #id}, and must be of type "cjmiam".
         *
         * <p>The consequence {@code Map} for a {@code InternalMessage} is required to have valid
         * values for the following fields:
         *
         * <ul>
         *   <li>{@value MessagingConstants.EventDataKeys.RulesEngine#MESSAGE_CONSEQUENCE_ID} -
         *       {@code String} containing the message ID
         *   <li>{@value MessagingConstants.EventDataKeys.RulesEngine#MESSAGE_CONSEQUENCE_TYPE} -
         *       {@code String} containing the consequence type
         *   <li>{@value MessagingConstants.EventDataKeys.RulesEngine#MESSAGE_CONSEQUENCE_DETAIL} -
         *       {@code Map<String, Object>} containing details of the Message to be displayed
         * </ul>
         *
         ...
         * @param rawInAppMessageSettings {@code Map<String, Object>} containing the raw message
         *     settings found in the "mobileParameters" present in the rule consequence
         * @param assetMap {@code Map<String, Object>} containing a mapping of a remote image asset
         *     URL and it's cached location
         * @throws MessageRequiredFieldMissingException if the consequence {@code Map} fails
         *     validation.
         */
        private InternalMessage(
            ...
            if (StringUtils.isNullOrEmpty(id)) {
                Log.debug(
                        ...
                        SELF_TAG,
                        "Invalid consequence (%s). Required field \"id\" is null or empty.",
            ...
            if (MapUtils.isNullOrEmpty(details)) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Invalid consequence (%s). Required field \"detail\" is null or empty.",
                        consequence.toString());
                throw new MessageRequiredFieldMissingException(
            ...
            if (!MessagingConstants.SchemaValues.SCHEMA_IAM.equals(schemaType)) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Invalid consequence (%s). Required field \"schema\" is (%s) should be of"
                                + " type (%S).",
                        consequence.toString(),
                        schemaType,
                        MessagingConstants.SchemaValues.SCHEMA_IAM);
                throw new MessageRequiredFieldMissingException(
                        "Required field: \"schema\" is not equal to"
            ...
            final Map<String, Object> data =
                    DataReader.optTypedMap(
                            ...
                            details,
                            MessagingConstants.EventDataKeys.RulesEngine
                                    .MESSAGE_CONSEQUENCE_DETAIL_KEY_DATA,
                            null);
            if (MapUtils.isNullOrEmpty(data)) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Invalid consequence (%s). Required field \"data\" is null or empty.",
                        consequence.toString());
                throw new MessageRequiredFieldMissingException(
                        "Required field: \"data\" is null or empty.");
            }
            ...
            final String html =
                    DataReader.optString(
                            data,
                            MessagingConstants.EventDataKeys.RulesEngine
                                    .MESSAGE_CONSEQUENCE_DETAIL_KEY_CONTENT,
                            "");
            if (StringUtils.isNullOrEmpty(html)) {
                ...
                throw new MessageRequiredFieldMissingException(
                        "Required field: \"html\" is null or empty.");
            }

            final InAppMessageSettings settings =
                    InAppMessageSettingsFromMap(rawInAppMessageSettings, html, assetMap);
            ...
            if (uiService == null) {
                Log.warning(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "The UIService is unavailable. Aborting in-app message creation.");
                throw new IllegalStateException("The UIService is unavailable");
            }
            aepMessage =
                    uiService.create(
                            new InAppMessage(settings, new MessagingFullscreenEventListener()),
         ...
         * @param eventType {@link MessagingEdgeEventType} enum containing the Event Type to be used
         *     for the ensuing Edge Event
         */
        public void track(final String interaction, final MessagingEdgeEventType eventType) {
            if (eventType == null) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Unable to send a proposition interaction, MessagingEdgeEventType was"
                                + " null.");
                return;
            }
            if (propositionInfo == null) {
                Log.debug(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Unable to send a proposition interaction (%s), PropositionInfo is not"
                                + " found for message (%s)",
            ...
            final Map<String, Object> propositionInteractionXdm =
                    propositionInteraction.getPropositionInteractionXDM();
        ...
        /**
         * Called when a {@code Message} is triggered - i.e. it's conditional criteria have been
         * met.
         */
        void trigger() {
            if (aepMessage != null) {
         ...
         * "mobileParameters": { "schemaVersion": "1.0", "width": 80, "height": 50, "verticalAlign":
         * "center", "verticalInset": 0, "horizontalAlign": "center", "horizontalInset": 0,
         * "uiTakeover": true, "displayAnimation": "top", "dismissAnimation": "top",
         * "backdropColor": "000000", // RRGGBB "backdropOpacity: 0.3, "cornerRadius": 15,
         * "gestures": { "swipeUp": "adbinapp://dismiss", "swipeDown": "adbinapp://dismiss",
         * "swipeLeft": "adbinapp://dismiss?interaction=negative", "swipeRight":
         * "adbinapp://dismiss?interaction=positive", "tapBackground": "adbinapp://dismiss" } } }
         */
        private InAppMessageSettings InAppMessageSettingsFromMap(
            ...
            final int height =
                    DataReader.optInt(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.HEIGHT,
                            FILL_SCREEN);
            final InAppMessageSettings.MessageAlignment verticalAlign =
                    InAppMessageSettings.MessageAlignment.valueOf(
                            (DataReader.optString(
                                            rawSettings,
            ...
            final int verticalInset =
                    DataReader.optInt(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.VERTICAL_INSET,
                            0);
            final InAppMessageSettings.MessageAlignment horizontalAlign =
                    InAppMessageSettings.MessageAlignment.valueOf(
                            (DataReader.optString(
                                            rawSettings,
                                            MessagingConstants.EventDataKeys.MobileParametersKeys
            ...
            final int horizontalInset =
                    DataReader.optInt(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.HORIZONTAL_INSET,
                            0);
            final InAppMessageSettings.MessageAnimation displayAnimation =
                    InAppMessageSettings.MessageAnimation.valueOf(
                            (DataReader.optString(
                                            rawSettings,
                                            MessagingConstants.EventDataKeys.MobileParametersKeys
                                                    .DISPLAY_ANIMATION,
                                            "none")
                                    .toUpperCase()));
            final InAppMessageSettings.MessageAnimation dismissAnimation =
                    InAppMessageSettings.MessageAnimation.valueOf(
                            (DataReader.optString(
                                            rawSettings,
                                            MessagingConstants.EventDataKeys.MobileParametersKeys
            ...
            final String backdropColor =
                    DataReader.optString(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.BACKDROP_COLOR,
                            "#FFFFFF");
            final float backdropOpacity =
                    DataReader.optFloat(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.BACKDROP_OPACITY,
                            0.0f);
            final float cornerRadius =
                    DataReader.optFloat(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.CORNER_RADIUS,
                            0.0f);
            final boolean uiTakeover =
            ...
            final Map<String, String> gestureMap =
                    DataReader.optStringMap(
                            rawSettings,
                            MessagingConstants.EventDataKeys.MobileParametersKeys.GESTURES,
                            new HashMap<>());
