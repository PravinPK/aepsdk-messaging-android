...
public class PropositionItem implements Serializable {
    ...
    public PropositionItem(
            final String itemId, final SchemaType schema, final Map<String, Object> itemData) {
        this.itemId = itemId;
        this.schema = schema;
    ...
    public static PropositionItem fromRuleConsequence(final RuleConsequence consequence) {
        ...
        try {
            ...
            Log.trace(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Exception occurred creating MessagingPropositionItem from rule consequence:"
                            + " %s",
    ...
    private SchemaData createSchemaData(final SchemaType schemaType) {
        if (MapUtils.isNullOrEmpty(itemData)) {
            Log.trace(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Cannot decode content, MessagingPropositionItem data is null or empty.");
            return null;
        }
    ...
    /**
     * Creates a {@code MessagingPropositionItem} object from the provided {@code Map<String,
     * Object>}.
     *
        ...
        try {
            ...
            if (MapUtils.isNullOrEmpty(dataMap)) {
                Log.trace(
                        MessagingConstants.LOG_TAG,
                        SELF_TAG,
                        "Cannot create MessagingPropositionItem, event data is null or empty.");
                return null;
            }
            ...
            Log.trace(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Exception caught while attempting to create a MessagingPropositionItem from an"
                            + " event data map: %s",
    ...
    public Map<String, Object> toEventData() {
        ...
        if (MapUtils.isNullOrEmpty(itemData)) {
            Log.trace(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "MessagingPropositionItem content is null or empty, cannot create event data"
                            + " map.");
            return eventData;
        }
