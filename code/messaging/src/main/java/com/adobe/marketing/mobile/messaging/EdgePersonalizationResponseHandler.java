...
class EdgePersonalizationResponseHandler {
            ...
            for (final Map.Entry<Surface, List<PropositionItem>> entry :
                ...
                for (final PropositionItem propositionItem : entry.getValue()) {
                    final PropositionInfo propositionInfo =
                            this.propositionInfo.get(propositionItem.getPropositionItemId());
     ...
     *
     * @param triggeredConsequence A {@link RuleConsequence} containing an in-app message
     *     definition.
     */
    void createInAppMessage(final RuleConsequence triggeredConsequence) {
        if (triggeredConsequence == null) {
            Log.debug(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Unable to create an in-app message, consequences are null.");
            return;
        }
        ...
        if (MapUtils.isNullOrEmpty(consequenceDetails)) {
            Log.warning(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Unable to create an in-app message, the consequence details are null or"
                            + " empty");
            return;
        }
        ...
        if (StringUtils.isNullOrEmpty(consequenceType)) {
            Log.debug(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Unable to create an in-app message, missing consequence type.");
            return;
        }
        ...
        if (!consequenceType.equals(MessagingConstants.SchemaValues.SCHEMA_IAM)) {
            Log.debug(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Unable to create an in-app message, unknown message consequence type: %s.",
                    consequenceType);
            return;
        }

        try {
            final Map<String, Object> detailsDataMap =
            ...
            final Map<String, Object> mobileParameters =
                    DataReader.optTypedMap(
                            Object.class,
                            detailsDataMap,
                            MessagingConstants.EventDataKeys.RulesEngine
                                    .MESSAGE_CONSEQUENCE_DETAIL_KEY_MOBILE_PARAMETERS,
                            Collections.emptyMap());
